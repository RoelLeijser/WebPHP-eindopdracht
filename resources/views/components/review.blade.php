@props(['review'])  
    <div class="border border-gray-300 p-4 mb-4 rounded-lg w-full">
        <div dusk="review" class="grid grid-cols-2">
            <div>
            ...
            @can('change review')
                <div class="flex justify-end space-x-3">
                    @if(Auth::check() && auth()->user()->id == $review->user_id)
                        <x-secondary-button dusk="update" class="h-8"><a href="{{route('review.edit', $review->id)}}">{{ __('review.update_button') }}</a></x-secondary-button>
                    @endif
